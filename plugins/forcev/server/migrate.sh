...
# File di stato per il progressivo (letto dal tool MCP 'stato_migrazione').
STATUS="$HOME/.forcev/migrate-status.json"
write_status() { # $1=phase — usa TGT_CACHE/TGT_VCACHE/CURVE_TOT/VIDEO_TOT se definiti
  mkdir -p "$HOME/.forcev" 2>/dev/null
  printf '{"phase":"%s","tgtCache":"%s","tgtVCache":"%s","curveTot":%s,"videoTot":%s}\n' \
    "$1" "${TGT_CACHE:-}" "${TGT_VCACHE:-}" "${CURVE_TOT:-0}" "${VIDEO_TOT:-0}" > "$STATUS" 2>/dev/null || true
}
...
CURVE_TOT=$(find "$ICLOUD_FL/ForceData" -type f 2>/dev/null | wc -l | tr -d ' ')
VIDEO_TOT=$(find "$ICLOUD_FL/ForceVideos" -type f 2>/dev/null | wc -l | tr -d ' ')
write_status "curve"
